...
- **单张**：指上去，右上角（告示与索引里是右侧）一个 ×。自己钉的是直接从名单里删；
  自动来的记进「已移除」，往后不再露面。摘掉之后**后面的顺移上来**。
- **一次摘好几张**：点墙底下那个「整理」。每张签换上勾选框，点一张勾一张
  （这时候点签不会跳走），选完「移除所选」。还有「全选 / 全不选」。
- **顺手关标签页**：整理时如果勾中的里面有标签页那一路，会多出一个
  「关掉所选标签页」。**× 只管墙面，这个才真的动浏览器** —— 分成两个按钮就是为了不误点。
- **放回**：设置 → 呈现 → 暂歇 → **已移除的站点**，一条条放回或者一次全放回。
- **永不再现**：「已移除」每行末尾还有个 ×，点了就把这个站点挪进**永不再现**。
  区别只有一条，但很实用 —— **「全部放回」碰不到永不再现那一份**：
  常摘常放的归前者，一辈子不想见的归后者。那一份也能一条条解除或全部解除。
